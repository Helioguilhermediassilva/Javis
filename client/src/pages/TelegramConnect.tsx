...
import { ArrowLeft, Bot, CheckCircle2, Copy, ExternalLink, Loader2, LogOut, MessageCircle, Power, ShieldCheck, Unplug, AlertTriangle } from "lucide-react";
...
export default function TelegramConnect() {
  ...
  async function connect() {
    ...
    setFeedback(null);
    try {
      const result = await xavierApi<{ connection: TelegramStatus["connection"] }>("/api/telegram/connect", { method: "POST", body: JSON.stringify({ botToken: token.trim() }) });
      setToken("");
      setFeedback({ type: "success", text: `@${result.connection?.bot_username || "Xavier"} conectado. O webhook já foi registrado automaticamente.` });
      await loadStatus();
    } catch (error) {
    ...
    <main className="min-h-screen overflow-y-auto bg-[#00060a] px-4 py-8 text-[#8ffcff] sm:px-8">
      <div className="mx-auto max-w-5xl">
        <header className="mb-8 flex flex-wrap items-center justify-between gap-4 border-b border-[#0d3347] pb-5">
          <div className="flex items-center gap-3"><Bot className="h-7 w-7 text-[#00d4ff]" /><div><div className="text-sm font-bold tracking-[0.3em] text-[#d8f8ff]">XAVIER / TELEGRAM LINK</div><div className="mt-1 text-[10px] tracking-[0.18em] text-[#3a8a9a]">CONTA {user?.email || "AUTENTICADA"}</div></div></div>
        ...
        <div className="grid gap-6 lg:grid-cols-[1.1fr_0.9fr]">
          <section className="border border-[#0d3347] bg-[#010d14] p-6 shadow-[0_0_40px_rgba(0,212,255,.06)] sm:p-8">
            <div className="mb-8 flex items-start justify-between gap-4"><div><p className="text-[10px] tracking-[0.22em] text-[#ff6b00]">BOT CONNECTION</p><h1 className="mt-2 text-2xl font-semibold text-[#d8f8ff]">Seu canal Telegram</h1><p className="mt-2 max-w-xl text-sm leading-6 text-[#5ab8cc]">Cada conta conecta o próprio bot. O token é enviado diretamente ao backend, cifrado antes de ser armazenado e nunca reaparece na tela.</p></div><MessageCircle className="hidden h-10 w-10 text-[#00d4ff]/60 sm:block" /></div>
            ...
            {loading ? <div className="flex items-center gap-3 border border-[#0d3347] p-5 text-xs text-[#5ab8cc]"><Loader2 className="h-4 w-4 animate-spin" /> CONSULTANDO STATUS...</div> : status?.connected ? <div className="border border-[#00ff88]/40 bg-[#00ff88]/5 p-5"><div className="flex flex-wrap items-center justify-between gap-3"><div><div className="flex items-center gap-2 text-sm font-semibold text-[#8dffc2]"><CheckCircle2 className="h-4 w-4" /> CONECTADO</div><div className="mt-2 text-lg text-[#d8f8ff]">@{status.connection?.bot_username || status.connection?.bot_display_name || "Xavier"}</div><div className="mt-1 text-xs text-[#5ab8cc]">Webhook ativo e separado para a sua conta.</div></div><button type="button" disabled={busy} onClick={() => void disconnect()} className="flex items-center gap-2 border border-[#ff3355]/50 px-3 py-2 text-[10px] uppercase tracking-[0.14em] text-[#ff9aac] transition hover:bg-[#ff3355]/10 disabled:opacity-50"><Unplug className="h-3.5 w-3.5" /> Desconectar</button></div><div className="mt-5 grid gap-3 border-t border-[#00ff88]/20 pt-4 text-[10px] text-[#5ab8cc] sm:grid-cols-2"><div><span className="text-[#3a8a9a]">PENDÊNCIAS TELEGRAM</span><div className="mt-1 text-[#d8f8ff]">{status.webhook?.pending_update_count ?? 0}</div></div><div><span className="text-[#3a8a9a]">ÚLTIMA VERIFICAÇÃO</span><div className="mt-1 text-[#d8f8ff]">{status.connection?.last_verified_at ? new Date(status.connection.last_verified_at).toLocaleString("pt-BR") : "—"}</div></div></div>{(status.webhook?.last_error_message || status.error) && <div className="mt-4 border-t border-[#ff3355]/20 pt-3 text-xs text-[#ff9aac]">{status.webhook?.last_error_message || status.error}</div>}</div> : <div><label className="block"><span className="mb-2 block text-[10px] uppercase tracking-[0.2em] text-[#3a8a9a]">Token do @BotFather</span><input type="password" value={token} onChange={(event) => setToken(event.target.value)} className="w-full border border-[#0d3347] bg-[#00060a] px-4 py-3 font-mono text-sm text-[#d8f8ff] outline-none transition placeholder:text-[#28596a] focus:border-[#00d4ff]" placeholder="123456789:AA..." autoComplete="off" /></label><button type="button" disabled={busy} onClick={() => void connect()} className="mt-4 flex w-full items-center justify-center gap-2 bg-[#00d4ff] px-4 py-3 text-xs font-semibold uppercase tracking-[0.18em] text-[#00060a] transition hover:bg-[#8ffcff] disabled:cursor-wait disabled:opacity-60">{busy ? <Loader2 className="h-4 w-4 animate-spin" /> : <Power className="h-4 w-4" />} Conectar bot</button><div className="mt-4 flex items-center gap-2 text-[10px] leading-5 text-[#3a8a9a]"><ShieldCheck className="h-4 w-4 shrink-0 text-[#00ff88]" /> O token não é salvo no navegador.</div></div>}
          </section>
