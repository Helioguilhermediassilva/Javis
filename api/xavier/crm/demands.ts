...
function requireId(req: VercelRequest): string {
  ...
  if (!/^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i.test(id)) {
    throw new XavierCrmValidationError("id da demanda inválido");
  }
  return id;
}
...
export default async function handler(req: VercelRequest, res: VercelResponse): Promise<void> {
  ...
  try {
    ...
    if (req.method === "GET") {
      res.status(200).json({ demands: await listXavierCrmDemands(user.id, queryValue(req, "status")) });
      return;
    }
    if (req.method === "POST") {
      const demand = await createXavierCrmDemand(user.id, await readBody(req) as XavierCrmDemandInput);
      res.status(201).json({ demand });
      return;
    }
    const id = requireId(req);
    if (req.method === "PATCH") {
      const demand = await updateXavierCrmDemand(user.id, id, await readBody(req) as XavierCrmDemandInput);
      res.status(200).json({ demand });
      return;
    }
    await deleteXavierCrmDemand(user.id, id);
    res.status(204).end();
  } catch (error) {
    const authError = authErrorResponse(error);
    const validation = error instanceof XavierCrmValidationError;
    res.status(authError?.status || (validation ? 400 : 502)).json({
      error: authError?.message || (error as Error).message || "Não foi possível acessar as demandas",
    });
  }
}
